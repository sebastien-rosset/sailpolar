...
class PolarAnalyzer:
    ...
    def analyze_data(self, df: pd.DataFrame) -> PolarData:
        ...
        polar_data = PolarData(
            ...
            conditions={"current": current, "instrument_health": instrument_health},
        )

        return polar_data

    def compare_with_manufacturer(
        self, measured: PolarData, manufacturer: PolarData
    ) -> Dict:
        ...
        if comparison["point_comparison"]:
            ...
            comparison["overall_achievement"] = sum(weighted_achievements) / sum(
                weights
            )
    ...
    def _correct_wind_data(
        self, df: pd.DataFrame, health: InstrumentHealth
    ) -> pd.DataFrame:
    ...
    def _estimate_speed_correction_factor(self, df: pd.DataFrame) -> float:
        ...
        valid_conditions = (df["STW"] > 2) & (  # Minimum speed for reliable readings
            abs(df["AWA"]).between(60, 120)
        )  # Beam reaching
    ...
    def _analyze_performance_patterns(self, comparison_points: List[Dict]) -> Dict:
        """Analyze patterns in performance comparison."""
        analysis = {"tws_patterns": {}, "twa_patterns": {}, "recommendations": []}
        ...
        for tws_range in tws_ranges.unique():
            tws_points = [
                p for p, r in zip(comparison_points, tws_ranges) if r == tws_range
            ]
            if tws_points:
        ...
        for twa_range in twa_ranges.unique():
            twa_points = [
                p for p, r in zip(comparison_points, twa_ranges) if r == twa_range
            ]
            if twa_points:
        ...
        analysis["recommendations"] = self._generate_performance_recommendations(
            analysis["tws_patterns"], analysis["twa_patterns"]
        )

        return analysis

    def _generate_performance_recommendations(
        self, tws_patterns: Dict[str, float], twa_patterns: Dict[str, float]
    ) -> List[str]:
