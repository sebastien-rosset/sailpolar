...
class ROTAnalyzer:
    ...
    def analyze_segment(self, segment: Segment):
        ...
        for sentence in segment.sentences:
            ...
            if heading is not None:
                heading_data.append(
                    {"timestamp": timestamp, "heading": heading, "source": source}
                )
    ...
    def cluster_rot_patterns(self, rot_data):
        ...
        df["rot_rolling_mean"] = (
            df["rot"].rolling(window=window_samples, center=True).mean()
        )
        df["rot_rolling_std"] = (
            df["rot"].rolling(window=window_samples, center=True).std()
        )
