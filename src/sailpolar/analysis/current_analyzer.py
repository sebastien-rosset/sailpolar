...
class CurrentAnalyzer:
    ...
    def estimate_current(
        self,
        df: pd.DataFrame,
        tidal_predictions: Optional[Dict] = None
        ...
        if tidal_predictions is not None:
            tidal_estimate = self._get_tidal_prediction(
                df.index[0],
                tidal_predictions
            )
            if tidal_estimate:
    ...
    def _has_required_data(self, df: pd.DataFrame, columns: List[str]) -> bool:
        ...
        return all(col in df.columns for col in columns) and not df[columns].isna().any().any()

    def _estimate_from_vector_difference(self, df: pd.DataFrame) -> Optional[CurrentEstimate]:
        """
        ...
        confidence = max(
            ...
            1.0 - min(variation / current_speed, 0.8)
        )
    ...
    def _estimate_from_turns(self, df: pd.DataFrame) -> Optional[CurrentEstimate]:
        ...
        confidence = self.config.min_confidence + (1.0 - self.config.min_confidence) / (1.0 + result.fun)
    ...
    def _extract_turn_segments(self, df: pd.DataFrame) -> List[pd.DataFrame]:
        ...
        for idx, row in df.iterrows():
            if last_timestamp is None or (idx - last_timestamp) <= self.config.max_time_gap:
    ...
    def _get_tidal_prediction(
        ...
        nearest_time = min(predictions.keys(), 
    ...
    def _calculate_vector_agreement(self, estimates: List[CurrentEstimate]) -> float:
        ...
        vectors = [(est.speed * np.cos(np.radians(est.direction)),
