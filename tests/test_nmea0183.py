...
def test_missing_fields():
    ...
    parsed = parser.parse_sentence(sentence.replace("XX", calculate_checksum(sentence[1:sentence.index('*')])))
    assert isinstance(parsed, RMCSentence)
...
def test_file_parsing_fail_unknown():
    ...
    assert "Unsupported sentence type" in str(exc.value)
...
def test_file_parsing_skip_unknown():
    ...
    for sentence in sentences:
        assert isinstance(sentence, (RMCSentence, VHWSentence, MWVSentence)), \
